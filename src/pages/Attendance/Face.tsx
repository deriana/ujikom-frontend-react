  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950 p-4 md:p-8 flex flex-col items-center font-sans text-zinc-900 dark:text-zinc-100 transition-colors">
      ...
      <div className="w-full max-w-4xl flex justify-between items-end mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight uppercase">Bulk Attendance System</h1>
          <p className="text-sm opacity-60 flex items-center gap-2">
            <span className={`w-2 h-2 rounded-full ${isModelsLoaded ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`}></span>
            System Status: {isModelsLoaded ? 'Live Monitoring' : 'Initializing AI...'}
          </p>
        </div>
      ...
      <div className="relative w-full max-w-4xl aspect-video bg-black rounded-2xl overflow-hidden shadow-2xl border border-zinc-200 dark:border-zinc-800">
        ...
        <div className="absolute top-8 left-10 z-20 flex items-center gap-2">
          ...
          <span className="text-white text-xs font-bold tracking-widest drop-shadow-md">REC</span>
        </div>
        ...
        <div className="absolute bottom-0 inset-x-0 bg-zinc-900/60 backdrop-blur-md border-t border-white/10 p-4 z-20 flex justify-between items-center">
          <div className="flex gap-6">
            <div className="flex flex-col">
              <span className="text-[10px] text-zinc-400 uppercase tracking-tighter">Coordinates</span>
              <span className="text-xs text-white font-mono">
                {coords ? `${coords.lat.toFixed(4)}, ${coords.lng.toFixed(4)}` : 'Searching...'}
              </span>
            </div>
            <div className="flex flex-col">
              <span className="text-[10px] text-zinc-400 uppercase tracking-tighter">Connection</span>
              <span className="text-xs text-white font-mono uppercase tracking-widest">Secure_Ujikom_v1</span>
            </div>
          </div>
        ...
        {!isModelsLoaded && (
          <div className="absolute inset-0 bg-zinc-950 z-50 flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-zinc-400 font-mono text-xs uppercase tracking-widest">Booting AI Engine...</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-4xl">
        <div className="p-4 bg-zinc-200/50 dark:bg-zinc-900/50 rounded-xl border border-zinc-200 dark:border-zinc-800">
            <p className="text-[10px] uppercase font-bold text-zinc-500">Method</p>
        ...
        <div className="p-4 bg-zinc-200/50 dark:bg-zinc-900/50 rounded-xl border border-zinc-200 dark:border-zinc-800 text-center">
            <p className="text-[10px] uppercase font-bold text-zinc-500">Auto Scan</p>
        ...
        <div className="p-4 bg-zinc-200/50 dark:bg-zinc-900/50 rounded-xl border border-zinc-200 dark:border-zinc-800 text-right">
            <p className="text-[10px] uppercase font-bold text-zinc-500">Security Level</p>
            <p className="text-sm font-semibold text-emerald-600 dark:text-emerald-400">High Reliability</p>
        </div>
      </div>
    </div>
  );
