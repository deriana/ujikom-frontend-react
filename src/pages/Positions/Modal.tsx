...
import { Briefcase, Wallet, Plus, Trash2, Users, Layers } from "lucide-react";
...
export default function PositionModal({
  ...
  return (
    <CrudModal
      ...
      onClose={onClose}
      onSubmit={onSubmit}
      title={isEdit ? "Update Position" : "Create New Position"}
      description={
        isEdit
          ...
          {positionData.allowances.length === 0 ? (
            <div className="text-center py-10 border-2 border-dashed border-gray-100 dark:border-gray-800 rounded-2xl bg-gray-50/30 dark:bg-gray-800/10">
              <div className="flex flex-col items-center gap-2 text-gray-400">
                ...
                <p className="text-sm">No allowances assigned yet</p>
              </div>
            </div>
          ) : (
            <div className="grid gap-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
              {positionData.allowances.map((allowance, index) => (
                <div
                  key={index}
                  className="group flex flex-col md:flex-row items-start md:items-center gap-3 p-3 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl hover:border-indigo-300 dark:hover:border-indigo-800 transition-all shadow-sm hover:shadow-md"
                >
                  <div className="flex-1 w-full">
                    <select
                      ...
                      className="w-full rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 text-sm font-medium outline-none focus:ring-2 focus:ring-indigo-500 dark:border-gray-700 dark:bg-gray-900 dark:text-white"
                    >
