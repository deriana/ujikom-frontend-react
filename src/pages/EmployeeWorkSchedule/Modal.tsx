...
import { Calendar, User, Briefcase, Info, ArrowRight, Clock9 } from "lucide-react";
...
interface EmployeeWorkScheduleModalProps {
  ...
  schedules: { uuid: string; name: string }[];
}
      ...
      <div className="relative w-full rounded-4xl bg-white p-1 dark:bg-gray-900 shadow-2xl overflow-hidden border border-gray-100 dark:border-gray-800">
        <div className="p-7">
          ...
          <div className="flex justify-between items-start mb-8">
            <div className="space-y-1">
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">
                {isEdit ? "Adjusting active period for the employee." : "Set up a new work policy assignment."}
              </p>
            </div>
          </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 gap-5 p-6 rounded-3xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-700/50 shadow-inner">
              <div className="space-y-2">
                ...
                <Select
                  ...
                  options={employees.map((e) => ({ label: e.name, value: e.nik }))}
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onChange={(val) => setData((prev) => ({ ...prev, work_schedule_uuid: val }))}
                  options={schedules.map((s) => ({ label: s.name, value: s.uuid }))}
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col md:flex-row items-center gap-3">
                <div className="w-full space-y-1.5">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Starts On</label>
                  <DatePicker
                    id="start-date-picker"
                    value={data.start_date}
                    onChange={(_, dateStr) => setData(p => ({ ...p, start_date: dateStr }))}
                ...
                <div className="w-full space-y-1.5">
                  <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Ends On (Optional)</label>
                  <DatePicker
                    id="end-date-picker"
                    value={data.end_date || ""}
                    onChange={(_, dateStr) => setData(p => ({ ...p, end_date: dateStr || null }))}
            ...
            {/* --- UX Context Box --- */}
            <div className={`transition-all duration-300 overflow-hidden ${!data.end_date ? 'max-h-20 opacity-100' : 'max-h-0 opacity-0'}`}>
              <div className="flex items-center gap-3 p-4 rounded-2xl bg-blue-50/50 dark:bg-blue-500/5 border border-blue-100 dark:border-blue-500/20">
                ...
                <p className="text-[11px] leading-relaxed text-blue-800/80 dark:text-blue-300/80 font-medium">
                  <strong>Permanent Assignment:</strong> Leaving the end date empty means this schedule stays active indefinitely.
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              >
                {isLoading ? (
                  ...
                  isEdit ? "Update Policy" : "Confirm Assignment"
                )}
              </Button>
            </div>
          </form>
