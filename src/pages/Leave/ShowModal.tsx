...
import { 
  Calendar, User, Briefcase, Info, Clock, 
  CheckCircle2, XCircle, Timer, FileText, Paperclip 
} from "lucide-react";
...
const StatusBadge = ({ status }: { status: ApprovalStatus }) => {
  const config = {
    0: { label: "Pending", color: "bg-yellow-100 text-yellow-700 border-yellow-200", icon: <Timer size={14} /> },
    1: { label: "Approved", color: "bg-green-100 text-green-700 border-green-200", icon: <CheckCircle2 size={14} /> },
    2: { label: "Rejected", color: "bg-red-100 text-red-700 border-red-200", icon: <XCircle size={14} /> },
  };
...
export default function LeaveShowModal({ uuid, isOpen, onClose }: LeaveShowModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} className="max-w-2xl m-4">
      <div className="relative w-full rounded-3xl bg-white p-8 dark:bg-gray-900 shadow-xl overflow-y-auto max-h-[90vh]">
        ...
        {isLoading ? (
          <div className="py-20 text-center animate-pulse text-gray-400">Fetching details...</div>
        ) : isError ? (
          <div className="p-4 bg-red-50 text-red-600 rounded-xl text-center">{(error as Error).message}</div>
        ) : leave && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
                ...
                <div className="flex items-center gap-2 text-gray-900 dark:text-white font-semibold">
                  ...
                  <span className="text-sm">{leave.date_start} - {leave.date_end}</span>
                </div>
                {leave.is_half_day && <span className="text-[10px] text-orange-500 font-bold mt-1 block">HALF DAY</span>}
              </div>

              <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Leave Balance</p>
                <div className="text-lg font-bold text-blue-600 dark:text-blue-400">
                  {leave.leave_balance?.remaining_days ?? 0} <span className="text-xs font-normal text-gray-500">days left</span>
                </div>
              </div>

              <div className="p-4 rounded-2xl bg-gray-50 dark:bg-gray-800/50 border border-gray-100 dark:border-gray-800">
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">Attachment</p>
                {leave.attachment?.exists ? (
                  <a href={leave.attachment.path} target="_blank" className="flex items-center gap-1 text-sm text-indigo-600 font-medium hover:underline">
              ...
              <div className="space-y-3">
                {leave.approvals.map((app, idx) => (
                  <div key={app.uuid} className="flex gap-4 relative">
                    ...
                    <div className={`z-10 w-8 h-8 rounded-full flex items-center justify-center border-4 border-white dark:border-gray-900 ${
                      app.status === 1 ? 'bg-green-500' : app.status === 2 ? 'bg-red-500' : 'bg-gray-300'
                    }`}>
                      {app.status === 1 ? <CheckCircle2 size={14} className="text-white" /> : <Clock size={14} className="text-white" />}
                    </div>
                    ...
                    <div className="flex-1 bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 p-3 rounded-xl shadow-sm">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="text-sm font-bold text-gray-800 dark:text-gray-200">{app.approver.name}</p>
                          <p className="text-[10px] text-gray-500 uppercase">{app.approver.role} (Level {app.level})</p>
                        </div>
                        <span className="text-[10px] text-gray-400 italic">{app.approved_at || 'Pending'}</span>
                      </div>
                      {app.note && <p className="mt-2 text-xs text-gray-600 dark:text-gray-400 border-t pt-2 border-gray-50 dark:border-gray-700 italic">"{app.note}"</p>}
                    </div>
                  </div>
                ))}
              </div>
            ...
            {leave.approval_status === 0 && leave.next_approver && (
              <div className="p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 flex items-center gap-2 text-xs text-blue-700 dark:text-blue-300">
                <Info size={14} />
                <span>Currently waiting for: <strong>{leave.next_approver}</strong></span>
              </div>
            )}
          </div>
        )}
