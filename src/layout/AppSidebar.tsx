...
const AppSidebar: React.FC = () => {
  ...
  useEffect(() => {
    ...
    ["main", "others"].forEach((menuType) => {
      ...
      items.forEach((nav, index) => {
        if (nav.subItems) {
          const matched = nav.subItems.some((sub) =>
  ...
  const isSubmenuActive = (nav: NavItem) => {
    ...
    return nav.subItems.some((sub) => location.pathname.startsWith(sub.path));
  };
          ...
          {nav.subItems ? (
            <button
              ...
              className={`menu-item group ${
                isSubmenuActive(nav) ? "menu-item-active" : "menu-item-inactive"
              } cursor-pointer ${
        ...
        <Link to="/">
          {isExpanded || isHovered || isMobileOpen ? (
            <>
              <SiteBrand
                className="py-2"
                logoClassName="h-12 rounded-lg"
                nameClassName="text-3xl text-primary"
              />
            </>
          ) : (
            <SiteBrand isCollapsed={true} logoClassName="rounded-lg" />
          )}
        </Link>
      </div>
