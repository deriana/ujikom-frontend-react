...
const MobileLayout = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 dark:bg-gray-900">
      {/* Sembunyikan Header Utama jika di halaman chat */}
      {!isChatPage && (
      ...
      {/* Connection Alert tetap muncul sebagai overlay jika error */}
      <ConnectionAlert 
        isOnline={isOnline} 
        isServerError={apiError || serverDown} 
      />

      {/* Jika halaman chat, hilangkan padding p-4 dan pb-24 agar chat memenuhi layar */}
      <main className={`flex-1 ${isChatPage ? "pb-0" : "pb-24 p-4"}`}>
        <Outlet />
      </main>

      {/* Sembunyikan Bottom Navbar jika di halaman chat */}
