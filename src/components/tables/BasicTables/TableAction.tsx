...
type ActionType = "delete" | "forceDelete" | "restore" | "archive" | "unarchive";
...
export default function TableActions<T extends string | number>({
  ...
  const actionConfig: Record<ActionType, { label: string; variant: string; icon: React.ReactNode }> = {
    delete: { label: "Delete", variant: "danger", icon: <Trash2 size={16} /> },
    forceDelete: { label: "Permanently Delete", variant: "destructive", icon: <Trash2 size={16} /> },
    restore: { label: "Restore", variant: "success", icon: <RotateCcw size={16} /> },
    archive: { label: "Archive", variant: "secondary", icon: <Archive size={16} /> },
    unarchive: { label: "Unarchive", variant: "secondary", icon: <XCircle size={16} /> },
  };

  const renderButton = (action: ActionType, handlerExists: boolean) =>
    handlerExists ? (
      <Tooltip key={action} content={`${actionConfig[action].label} ${dataName}`}>
        <Button
    ...
    <div className="flex items-center gap-2 justify-start">
      ...
      {onEdit &&
        <Tooltip content={`Edit ${dataName}`}>
          <Button onClick={() => onEdit(id)} aria-label="Edit" size="sm" variant="warning">
            <Pencil size={16} />
          </Button>
        </Tooltip>
      }
          ...
          <div className="no-scrollbar relative w-full max-w-100 overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-7">
            <div className="px-2 pr-14">
              ...
              <p className="mb-6 text-sm text-gray-500 dark:text-gray-400 lg:mb-7">
                Are you sure you want to {actionConfig[currentAction].label.toLowerCase()}{" "}
                <strong>{dataName || "this item"}</strong>? This action cannot be undone.
              </p>
            </div>
            ...
            <div className="flex items-center gap-3 px-2 mt-6 lg:justify-end">
              ...
              <Button size="sm" onClick={handleConfirmAction} variant={actionConfig[currentAction].variant as any}>
