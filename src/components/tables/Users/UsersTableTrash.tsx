...
import { useForceDeleteUser, useRestoreUser, useUsers } from "@/hooks/useUser";
import { RESOURCES } from "@/constants/Resource";
import { useMemo, useState } from "react";
import FilterDropdown from "@/components/FilterDropdown";
import { useTrashActions } from "@/hooks/useTrashActions";

export default function UsersTrashTable() {
  const { data: users = [], isLoading, isError, error } = useUsers(true);
  const [roleFilter, setRoleFilter] = useState("all");
  const [teamFilter, setTeamFilter] = useState("all");
  const [divisionFilter, setDivisionFilter] = useState("all");
  const [positionFilter, setPositionFilter] = useState("all");
  const { mutateAsync: restoreUser } = useRestoreUser();
  const { mutateAsync: forceDeleteUser } = useForceDeleteUser();
  ...
  const { handleRestore, handleForceDelete } = useTrashActions({
    label: "Users",
    restoreFn: restoreUser,
    forceDeleteFn: forceDeleteUser,
  });
