import {
  useAttendanceCorrectionApprovals,
  useAttendanceCorrectionApprovalsList,
} from "@/hooks/useAttendanceCorrection";
import { Column, AttendanceCorrection } from "@/types";
...
export default function AttendanceCorrectionApprovalTable() {
  const {
    data: corrections = [],
    isLoading,
    isError,
  ...
  const { mutateAsync: approveCorrection } = useAttendanceCorrectionApprovals();
  ...
  const handleApprovalAction = (
    ...
    handleMutation(
        ...
        }),
      {
        ...
        success: `Correction ${isApprove ? "approved" : "rejected"} successfully`,
        error: `Failed to ${isApprove ? "approve" : "reject"} correction`,
      },
    );
  };
    ...
    },
    {
      ...
      render: (row) => {
        return (
          <TableActions
            id={row.uuid || ""}
            dataName={`Correction - ${row.employee_name}`}
